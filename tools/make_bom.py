...
def main(name="bass viol+cello", out="BOM.md"):
    ...
    A("")
    A("**The cable channel through the carriage is missing.** His arm has a")
    A("3.5 mm core running its whole length and out through the front face of")
    A("the carriage, with a cone-shaped lead-in so the microphone lead is not")
    A("chafed on the edge. This rebuild has the core along the arm but stops")
    A("it short of the front face, so the lead has no way out. Verified and")
    A("not yet fixed.")
    A("")
